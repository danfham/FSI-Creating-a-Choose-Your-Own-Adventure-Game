//Nested if version of the basic Adventure game discussed in class. 

//intialize the adventure path via window prompt
let firstAnswer  = window.prompt('Do you head left or right?')

//begin intial decision of left
if(firstAnswer === 'left'){
    let secondAnswer = window.prompt(`You come across a stray cat. 
It scampers off down a small hole, just large enough for you to crawl through. 
Do you follow it, or continue on your path?`)
    if(secondAnswer === `Follow`){
      let secondDecision = window.prompt(`You follow the cat to a colony of cats, nestled in a fort of warm blankets and subsisting off of inexplicably warm soup. They are content with you staying, but you wonder if you should alert the world to this magical safe haven.`)
      if(secondDecision===`Stay`){
        window.alert(`You live happily amongst the cats for the rest of your days.`)
      } else if(secondDecision===`Spread the word`){
    ...
    } else if(firstDecision === `Continue`){
      let secondDecision = window.prompt(`You come across a chamber that reaches upward to a shining light above. There is a long, winding staircase, and a much quicker, but rickety-looking ladder that leads up toward the light. Which do you take?`)
      if(secondDecision === 'Ladder'){
...
lead you away from the dragon altogether. Which path do you take?`)
    if(secondAnswer===`Past the dragon`){
      ...
      let secondDecision = window.prompt(`After walking a while longer, you come across a shiny blue flower. It is so beautiful that you decide you must either draw it or pick it. Which do you do?`)
      if(secondDecision===`Draw it`){
      ...
      }else if(secondDecision==='Pick it'){
